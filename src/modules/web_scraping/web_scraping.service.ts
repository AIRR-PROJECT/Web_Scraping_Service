            ...
            try {
                ...
                const postsData: PostData[] = await page.evaluate(() => {
                    ...
                    return Array.from(posts).map(post => {
                        const linkPostElement: HTMLAnchorElement = post.querySelector("a.crayons-story__hidden-navigation-link");
                        ...
                        const reactionsElement: HTMLElement = statisticsElement.querySelector("div.crayons-story__details > a:nth-child(1) > div > span.aggregate_reactions_counter")
                        const cmtElement: HTMLElement = statisticsElement.querySelector("div.crayons-story__details > a:nth-child(2)")
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const article_Json: ArticleElement[] = await page.evaluate(() => {
                    ...
                    const createContainer = (tag: string, text: string) => ({
                        tag,
                        text: text.trim(),
                        children: [],
                    });
                ...
                let s = "";
                const traverseCollectElement = (aritcleJson: ArticleElement[], special) => {
